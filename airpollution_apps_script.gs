 ...
 *
 * ไฟล์นี้คุมเฉพาะส่วนรับแบบสอบถามและบันทึกลงชีต "response_r2" เท่านั้น
 * ส่วนสรุปสถิติ (Median / IQR) แยกไปอยู่ที่ไฟล์ stat2.gs + stat2.html ทั้งหมด
 * เพื่อไม่ให้กระทบระบบเก็บข้อมูลที่ทำงานอยู่ — ไฟล์นี้จึงไม่ต้องแก้อะไรเพิ่ม
 * นอกจากขั้นตอนติดตั้งปกติด้านล่าง
 *
 * วิธีติดตั้ง
 ...
 *
 * ดูวิธีติดตั้งหน้าสรุปสถิติ (stat2) ได้ที่ไฟล์ stat2.gs
 */

const SPREADSHEET_ID = 'PASTE_YOUR_SPREADSHEET_ID_HERE'; // จาก URL ของ Google Sheet
const SHEET_NAME = 'response_r2'; // ชีตเก็บคำตอบทั้งหมด (ต้นทาง ห้ามเปลี่ยนชื่อ/โครงสร้างโดยไม่จำเป็น)
 ...
 * รองรับ query param ?page=xxx สำหรับ serve หน้าอื่นในโปรเจกต์เดียวกัน เช่น ?page=stat2, ?page=delphi3
 *
 * หมายเหตุ: Apps Script อนุญาตให้มีฟังก์ชัน doGet() ได้เพียงจุดเดียวต่อโปรเจกต์
 * (ทุกไฟล์ .gs ในโปรเจกต์เดียวกันแชร์ namespace เดียวกัน) จึงต้องมี entry ของ
 * 'stat2' และ 'delphi3' อยู่ในไฟล์นี้ที่เดียว — เป็นเพียงการ "เพิ่ม" บรรทัดแมปหน้า
 * ไม่ได้แตะตรรกะการรับ/บันทึกแบบสอบถามรอบ 2 ใด ๆ ทั้งสิ้น ฟังก์ชันจริงของแต่ละ
 * หน้าอยู่ในไฟล์ stat2.gs และ delphi3.gs แยกต่างหากทั้งหมด
 */
function doGet(e) {
  const pageMap = {
    ...
    'delphi3': { file: 'delphi3', title: 'แบบสอบถามเทคนิคเดลฟาย รอบที่ 3' },
